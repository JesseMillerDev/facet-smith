...
`manifest` emits a deterministic schema-v2 JSON catalog containing each experiment's iteration, variants, revisions, resolver ID, optional allocation, implementation hashes, and source location. Imported source variants hash their implementation file; inline and locally declared variants hash their component declaration. A traffic-bearing implementation edit therefore produces reviewable drift even when its revision was not bumped. Test, build, coverage, and dependency directories are excluded from discovery.
